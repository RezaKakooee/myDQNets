...
class QNetwork():
    def __init__(self, state_shape, action_size):
        ...
        self.q_state_ation = tf.reduce_sum(tf.multiply(self.q_state, action_one_hot), axis=1)
        
        self.loss = tf.reduce_mean(tf.square(self.q_state_ation - self.q_target_in))
    ...
    def get_q_state(self, sess, state):
        q_state = sess.run(self.q_state, feed_dict={self.state_in:state})
        return q_state
